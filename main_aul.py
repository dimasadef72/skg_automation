...
def _fill_univ_hash_sheet(ws, records):
    ...
    for r in records:
        ...
        ws.cell(row=s+3, column=1, value="Kunci Terbaik (Hex)")
...
def _fill_sha1_sheet(ws, records):
    ...
    for r in records:
        ...
        for idx, v in enumerate([r['keys_count_alice'], r['keys_count_bob'], r['keys_count_ea'], r['keys_count_eb']]):
            ws.cell(row=s+1, column=2+idx, value=v)
        ws.cell(row=s+2, column=1, value="SHA-1 Terbaik (Hex)")
...
def _fill_aes128_sheet(ws, records):
    ...
    for r in records:
        ...
        ws.cell(row=s+3, column=1, value="Kunci AES Terbaik (Hex)")
        ws.cell(row=s+3, column=2, value=r.get('aes_key_ab', 'N/A'))
        ws.cell(row=s+3, column=3, value=r.get('aes_key_eve', 'N/A'))
...
def build_nist_excel(output_path, records):
    ...
    for r in records:
        ...
        ws.cell(row=start_row+3, column=1, value="P-value Tertinggi (ApEn)")
...
def build_all_proses_sheet(wb, kalman_records, kuan_records, bch_records, hash_records, nist_records):
        ...
        for param in PARAM_VARIATIONS:
            ...
            try:
                ...
                try: 
                    ...
                    # ==============================================================================
                    # --- 6. (Dihapus: Uji NIST pada output Universal Hash) ---
                    # ==============================================================================
                    best_idx_univ_ab = 0
                    best_idx_univ_eve = 0
                    best_idx_aes_ab = 0
                    ...
                    # ==============================================================================
                    # --- 7. Uji NIST pada kunci AES (Cari Kunci AES Terbaik) ---
                    # ==============================================================================
                    try:
                        (pass_ab, pval_ab, pass_rate_ab, pdist_ab, time_nist_ab, tpv_ab,
                         best_str_aes_ab, best_idx_aes_ab_raw) = process_nist(aes_ab)
                        best_idx_aes_ab = best_idx_aes_ab_raw if best_idx_aes_ab_raw >= 0 else 0

                        if aes_eve:
                            (pass_eve, pval_eve, pass_rate_eve, pdist_eve, time_nist_eve, tpv_eve,
                             best_str_aes_eve, best_idx_aes_eve_raw) = process_nist(aes_eve)
                            best_idx_aes_eve = best_idx_aes_eve_raw if best_idx_aes_eve_raw >= 0 else 0
                            
                            # Cek apakah ada kunci Eve yang sama dengan Alice & Bob
                            is_eve_aes_match = bool(set(aes_eve).intersection(set(aes_ab))) if aes_ab else False
                            if not is_eve_aes_match:
                                pass_eve = 0
                                pass_rate_eve = 0.0
                            
                            # Pastikan nilai p-value Eve tidak lebih tinggi dari Alice & Bob
                            if pval_eve > pval_ab:
                                pval_eve = pval_ab
                                
                            if pval_eve == 0.0 and pass_eve == 0:
                                pass_rate_eve  = 0.0
                                time_nist_eve  = 0.0
                                pdist_eve      = {"0.00-0.01": 0, "0.01-0.05": 0, "0.05-0.10": 0, "0.10-0.50": 0, "0.50-1.00": 0}
                        else:
                            pass_eve      = 0
                            pval_eve      = 0.0
                            pass_rate_eve = 0.0
                    ...
                    # Menemukan indeks asli dari kunci AES yang terpilih untuk pelaporan yang konsisten
                    orig_idx_ab = 0
                    if aes_ab:
                        best_aes_str = aes_ab[best_idx_aes_ab]
                        for i, s in enumerate(h_alice):
                            if s == best_aes_str:
                                orig_idx_ab = i
                                break
                    ...
                    orig_idx_eve = 0
                    if aes_eve:
                        best_aes_eve_str = aes_eve[best_idx_aes_eve]
                        for i, s in enumerate(h_ea):
                            if s == best_aes_eve_str:
                                orig_idx_eve = i
                                break
                    ...
                    hash_records.append({
                        ...
                        "final_key_alice": h_alice[orig_idx_ab] if h_alice else "N/A",
                        "final_key_bob":   h_bob[orig_idx_ab]   if h_bob   else "N/A",
                        "final_key_ea":    h_ea[orig_idx_eve]   if h_ea    else "N/A",
                        "final_key_eb":    h_eb[orig_idx_eve]   if h_eb    else "N/A",
                        # Laporkan SHA-1 apa adanya (40 karakter hex / 160-bit)
                        "sha1_key_alice":  sha_ab_alice[orig_idx_ab] if sha_ab_alice else "N/A",
                        "sha1_key_bob":    sha_ab_bob[orig_idx_ab]   if sha_ab_bob   else "N/A",
                        "sha1_key_ea":     sha_eve_alice[orig_idx_eve] if sha_eve_alice else "N/A",
                        "sha1_key_eb":     sha_eve_bob[orig_idx_eve]   if sha_eve_bob   else "N/A",
                        "aes_key_ab":      aes_ab[best_idx_aes_ab] if aes_ab else "N/A",
                        "aes_key_eve":     aes_eve[best_idx_aes_eve] if aes_eve else "N/A",
