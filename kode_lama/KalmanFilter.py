...
xaposteriori_0 = -5
paposteriori_0 = 1
bb = 6  # jumlah measurement per sample
interval = 0.11
...
rss_alice = read_rssi_csv('skenario1_mita_evealice.csv')
rss_bob   = read_rssi_csv('skenario1_mita_evebob.csv')
...
os.makedirs('Output/P2P/hasilpraproses', exist_ok=True)

# === Simpan hasil Kalman Alice ===
hasil_alice = np.array(kalman_filter(alice)).T.reshape(-1, 1)
with open('Output/P2P/hasilpraproses/eve_alice_skenario1.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
with open('Output/P2P/hasilpraproses/eve_bob_skenario1.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
excel_path = 'Output/P2P/hasilpraproses/analisis_kalman_eve.xlsx'
wb.save(excel_path)
